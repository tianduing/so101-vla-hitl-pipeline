...
- ACT、Diffusion Policy、SmolVLA 均已生成 100k 最终 checkpoint；这证明训练完成，不自动等于新的真机成功率。
...
1-step、62% 和 85.7% 的逐项通俗解释以及正式训练队列状态，见 [`TRAINING_AND_METRICS_EXPLAINED.md`](TRAINING_AND_METRICS_EXPLAINED.md)。
...
- 100k checkpoint 已存在，但没有连接真机所得到的新闭环结果；不能用训练步数、loss 或离线动作误差代替真机成功率。
